		...
		<div id="content">
			<div id="information" class="white-texture rounded clearfix">
				...
				<?php for($i=0;$i<count($images);$i++): ?>
					<div class="frames" id="ph<?=$i?>">
						<div id="id<?=$i?>" style="display:none;"><?=$images[$i]['id'];?></div>
						<?php $link = $baseurl.'material/viewimage/'.$images[$i]['id']; ?>
						<?php $text = '<img src="'.$baseurl.'material/viewthumb/'.$images[$i]['id'].'" 
									alt="'.$images[$i]['title'].'" '.'title="'.$images[$i]['title'].'"/>'; ?>
						<?php $attr = array('class'=>'pirobox','title'=>$images[$i]['title']); ?>
						<?= anchor($link,$text,$attr); ?>
						<br/>
						<div style="float:right;">
							<input type="image" title="Удалить" class="PhotoDel" nID="<?=$i;?>" src="<?=$baseurl;?>images/buttons/delete.png"/>
						</div>
					</div>
				<?php endfor; ?>
					</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".PhotoDel").click(function(){
				...
				var PhotoID = $("#id"+curID).text();
				$.post(
					"<?=$baseurl;?>admin/<?=$this->uri->segment(2);?>/manager/delete-photo",
					{'id':PhotoID},
					function(data){
						if(data.status){
							$("#ph"+curID).fadeOut("slow",function(){
								$("#ph"+curID).remove();
